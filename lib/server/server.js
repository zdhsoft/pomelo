    ...
    try {
        app.sysrpc[routeRecord.serverType].msgRemote.forwardMessage(
            ...
            function (err, resp, opts) {
                if (err) {
                    //这里加上msg输出，可以便于知道是哪个消息出错
                    logger.debug('---------------- forwardMessage has error0 ----------------begin');
                    logger.error('fail to process romote message: msg=', msg);
                    logger.error('fail to process remote message:' + err.stack);
                    logger.debug('---------------- forwardMessage has error0 ----------------end');
                }
                finished = true;
                utils.invokeCallback(cb, err, resp, opts);
            }
        ...
        if (!finished) {
            //这里加上msg输出，可以便于知道是哪个消息出错
            logger.debug('---------------- forwardMessage has error1 ----------------begin');
            logger.error('fail to forward message: msg=', msg);
            logger.error('fail to forward message:' + err.stack);
            logger.debug('---------------- forwardMessage has error1 ----------------end');
